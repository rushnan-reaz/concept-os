...
fn main() -> ! {
    ...
    // is never reached, so the kernel's 30-second REVERT_UPDATE_TIMEOUT will
    // fire and roll back to the previous component version.
    kipc::activate_task();
    ...
    loop {
        ...
        let current_temp = temp_result.unwrap_lite().clamp(-40.0, 125.0);
