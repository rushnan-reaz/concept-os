...
pub trait RelocatorMethods<T> {
    ...
    fn flush(&mut self, position: usize, src: &[u8], aux: &mut T) -> Result<(),()>;
}
    ...
    Relocator<LINKED_FLASH_ADDR, LINKED_SRAM_ADDR, BUFF_SIZE, RELOC_BUFF_SIZE>
{
    ...
    fn flush_working_buffer<T>(
        &mut self,
        num_bytes: usize,
        relocator_methods: &mut dyn RelocatorMethods<T>,
        aux: &mut T
    ) -> Result<usize,()> {
        ...
        while total_flushed < num_bytes {
            ...
            relocator_methods.flush(self.current_file_pos, &buff[0..total], aux)?;
    ...
    fn flush_bytes<T>(
        ...
        while total_flushed < num_bytes {
            ...
            total_flushed += self.flush_working_buffer(num_bytes - total_flushed, relocator_methods, aux)?;
    ...
    pub fn consume_current_buffer<T>(
        &mut self,
        current_buffer: &[u8],
        relocator_methods: &mut dyn RelocatorMethods<T>,
        aux: &mut T
        ...
        if self.relocation_buffer.no_more_relocs() {
            ...
            relocator_methods.flush(self.current_file_pos, current_buffer, aux)?;
        ...
        while self.current_buffer_pos < current_buffer.len() || self.working_buffer.read_capacity() > 0 {
            ...
            let next_reloc_option = self.relocation_buffer.get_next(relocator_methods, aux)?;
            if next_reloc_option.is_none() {
                // No more relocations
                //assert!(self.relocation_buffer.no_more_relocs());
                // Flush all we can
                self.flush_bytes(usize::MAX, current_buffer, relocator_methods, aux)?;
            ...
            if distance > current_buffer.len() + self.working_buffer.read_capacity() {
                ...
                self.flush_working_buffer(BUFF_SIZE, relocator_methods, aux)?;
                relocator_methods.flush(self.current_file_pos, &current_buffer[self.current_buffer_pos..current_buffer.len()], aux)?;
                // Advance counters!
            ...
            if !self.flush_bytes(distance, current_buffer, relocator_methods, aux)? {
    ...
    pub fn finish<T>(mut self, relocator_methods: &mut dyn RelocatorMethods<T>, aux: &mut T) -> Result<(),()> {
        // Flushes everything that remained in the working buffer
        self.flush_working_buffer(usize::MAX, relocator_methods, aux).map(|_| ())
    }
...
impl<'a, const N: usize> RelocationBuff<N> {
    ...
    fn refill<T>(&mut self, relocator_methods: &mut dyn RelocatorMethods<T>, aux: &mut T) -> Result<bool,()> {
        ...
        let _total_read = relocator_methods.read_relocations(self.total_used_relocs, &mut reloc_buff[0..to_transfer], aux)?;
    ...
    fn get_next_free(&mut self) -> Option<Relocation> {
        ...
        if self.buff.read_capacity() == 0 {
            ...
            return Some(*self.buff.peek(0).unwrap());
        }
    }
    pub fn get_next<T>(&mut self, relocator_methods: &mut dyn RelocatorMethods<T>, aux: &mut T) -> Result<Option<Relocation>,()> {
        // Check if refill is necessary
        if !self.refill(relocator_methods, aux)? && self.is_empty() {
        ...
        if !self.is_empty() {
            panic!("Relocation buffer too small");
        }
    ...
    pub fn consume_relocation(&mut self, reloc_index: usize) {
        self.buff.peek_mut(reloc_index).unwrap().set_consumed();
    }
}
